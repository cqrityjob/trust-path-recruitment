...
BEGIN
  ...
  BEGIN
    ...
    VALUES (
      ...
      -- own Swedish label.
      _t.name_sv,
      -- A governed issuer where the catalogue names one; where the organisation-role
      -- model says it is stated on the document, the provider the holder names.
      coalesce((SELECT issuer_name FROM public.sp_approved_credential_catalogue WHERE code=_t.code),
               'Fiktiv Training Provider Ltd'),
      _jur,
      _sub,
...
BEGIN
  ...
  -- ROUTE A (owner decision 2026-09-18, 20261126090000): the member of a pilot
  -- market saves that market's definitions. is_active stays false throughout.
  PERFORM pg_temp.ok(_r = 'OK',
    '2.1 a British licence can be saved as a draft by a GB pilot member (got ' || _r || ')');

  _r := pg_temp.file_from_taxonomy(_member, 'UK_SIA_LICENCE_DS', 'active');
  PERFORM pg_temp.ok(_r = 'OK',
    '2.2 and added to the Passport (got ' || _r || ')');
  ...
  PERFORM pg_temp.ok(_n = 2,
    '2.3 both are stored under Great Britain, with no submarket');

  _r := pg_temp.file_from_taxonomy(_member, 'AE_DU_SIRA_CARD_GUARD', 'active');
  PERFORM pg_temp.ok(_r = 'OK',
    '2.4 a scoped Dubai card is saved with its company by a Dubai pilot member (got ' || _r || ')');
  ...
  PERFORM pg_temp.ok(_n = 1,
    '2.5 and it is stored under AE with the emirate AE-DU');

  IF _has_ni THEN
    _r := pg_temp.file_from_taxonomy(_member, 'UK_SIA_LICENCE_VI', 'active');
    PERFORM pg_temp.ok(_r = 'OK',
      '2.6 the Northern Ireland licence is saved by a GB-NI pilot member (got ' || _r || ')');
    SELECT count(*) INTO _n FROM public.sp_claims
    ...
    PERFORM pg_temp.ok(_n = 1,
      '2.7 and it is stored under GB with the submarket GB-NI');
  ELSE
  ...
  PERFORM pg_temp.ok(_n = 0,
    '2.8 EVERY GB and Dubai pilot definition can be saved by its member, scoped and document-issuer ones included');
