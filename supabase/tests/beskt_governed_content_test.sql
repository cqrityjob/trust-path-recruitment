...
BEGIN;
\ir beskt_governed_content_fixture.sql
...
BEGIN
  ...
  -- A cycle needs a backward edge, which the write-time guard refuses; the
  -- rows are forced past it to prove the validator on the stored graph.
  PERFORM pg_temp.must_fail(
    format($q$INSERT INTO public.beskt_routing_rules (method_version_id, rule_key, evaluation_order, applies_mode, source_item_id, condition_kind, action, target_item_id)
             VALUES (%L, 'cyc2', 51, 'recruitment_support', %L, 'always', 'show', %L)$q$, b.rec_v, _i10, _i9),
    'BESKT_ROUTE_BACKWARD', 'B2.50b a rule whose target precedes its source is refused at write time');
  PERFORM pg_temp.must_fail(
    format($q$INSERT INTO public.beskt_routing_rules (method_version_id, rule_key, evaluation_order, applies_mode, source_item_id, condition_kind, action, target_item_id)
             VALUES (%L, 'self', 51, 'recruitment_support', %L, 'always', 'show', %L)$q$, b.rec_v, _i9, _i9),
    'BESKT_ROUTE_BACKWARD', 'B2.50c a rule targeting its own source is refused');
  PERFORM pg_temp.must_fail(
    format($q$INSERT INTO public.beskt_routing_rules (method_version_id, rule_key, evaluation_order, applies_mode, source_item_id, condition_kind, condition_boolean, action, target_item_id)
             VALUES (%L, 'back_across', 51, 'recruitment_support', %L, 'boolean_equals', true, 'show', %L)$q$, b.rec_v, b.rec_i3, b.rec_i1),
    'BESKT_ROUTE_BACKWARD', 'B2.50d a boolean rule cannot reopen an earlier item either');
  ALTER TABLE public.beskt_routing_rules DISABLE TRIGGER beskt_routing_rules_child_guard;
  ...
  PERFORM pg_temp.ok('ROUTE_TARGET_BEFORE_SOURCE' = ANY (pg_temp.blockers(b.rec_v)), 'B2.52a a forced backward edge blocks publication in the validator too');
  ...
  -- Hidden and skipped sources: a rule reads only an item that is currently
  -- shown, so a stale answer for a hidden or skipped item moves nothing. The
  -- rule that decides whether reported_incident is shown evaluates before the
  -- rule that reads it.
  UPDATE public.beskt_routing_rules SET evaluation_order = 10 WHERE method_version_id = b.rec_v AND rule_key = 'show_context_when_reported';
  ...
  VALUES (b.rec_v, 'show_incident_when_experienced', 3, 'recruitment_support', b.rec_i1, 'option_selected', b.rec_o_yes, 'show', b.rec_i3);
  PERFORM pg_temp.become('b2000000-0000-4000-8000-0000000000e1');
  SET LOCAL ROLE authenticated;
  SELECT string_agg(sequence_position || ':' || item_key, ',' ORDER BY sequence_position) INTO _a
    FROM public.beskt_resolve_item_sequence(b.rec_v, b.rec_p1, 'recruitment_support',
      '{"reported_incident": {"kind": "boolean", "value": true}}');
  ...
  RESET ROLE; PERFORM pg_temp.nobody();
  PERFORM pg_temp.ok(_a = '1:lone_working_experience,2:information_acknowledged',
    format('B2.52c a stale answer for a HIDDEN source reveals nothing downstream: reported_incident is not shown, so its answer cannot open incident_context (%s)', _a));
  PERFORM pg_temp.ok(_b2 = '1:lone_working_experience,2:lone_working_example,3:reported_incident,4:incident_context,5:information_acknowledged',
    format('B2.52d once its source is shown the same answer opens the follow-up (%s)', _b2));
  DELETE FROM public.beskt_routing_rules WHERE method_version_id = b.rec_v AND rule_key = 'show_incident_when_experienced';
  INSERT INTO public.beskt_routing_rules (method_version_id, rule_key, evaluation_order, applies_mode, source_item_id, condition_kind, condition_option_id, action, target_item_id)
  VALUES (b.rec_v, 'skip_incident_when_inexperienced', 3, 'recruitment_support', b.rec_i1, 'option_selected', b.rec_o_no, 'skip', b.rec_i3);
  PERFORM pg_temp.become('b2000000-0000-4000-8000-0000000000e1');
  SET LOCAL ROLE authenticated;
  SELECT string_agg(sequence_position || ':' || item_key, ',' ORDER BY sequence_position) INTO _a
    FROM public.beskt_resolve_item_sequence(b.rec_v, b.rec_p1, 'recruitment_support',
  ...
  RESET ROLE; PERFORM pg_temp.nobody();
  PERFORM pg_temp.ok(_a = '1:lone_working_experience,2:information_acknowledged',
    format('B2.52e a stale answer for a SKIPPED source moves nothing: reported_incident was skipped, so its answer cannot open incident_context (%s)', _a));
  DELETE FROM public.beskt_routing_rules WHERE method_version_id = b.rec_v AND rule_key = 'skip_incident_when_inexperienced';
  ...
  PERFORM pg_temp.ok('ITEM_INSTRUCTS_SCORING' = ANY (pg_temp.blockers(b.rec_v)), 'B2.52g an item wording that instructs a suitability rating blocks publication');
  UPDATE public.beskt_items SET wording_en = 'Describe the context.', wording_sv = 'Betygsätt kandidatens lämplighet på en skala 1–5.' WHERE id = b.rec_i4;
  PERFORM pg_temp.ok('ITEM_INSTRUCTS_SCORING' = ANY (pg_temp.blockers(b.rec_v)), 'B2.52h the same instruction in Swedish blocks publication');
  ...
  PERFORM pg_temp.ok(
    ...
    AND public.beskt_text_instructs_scoring('Is this candidate suitable?')
    AND public.beskt_text_instructs_scoring('Ge poäng för varje exempel.'),
    'B2.52o the detector reads instructions to score, rate, grade, rank or judge suitability, not ordinary interview wording');
...
BEGIN
  ...
  PERFORM pg_temp.ok(
    (SELECT count(*) FROM pg_indexes WHERE schemaname = 'public' AND tablename = 'beskt_method_versions'
      ...
      AND indexdef LIKE '%(pack_id, open_slot)%') = 1
    AND (SELECT count(*) FROM pg_index i JOIN pg_class c ON c.oid = i.indrelid
          WHERE c.relname = 'beskt_method_versions' AND i.indisunique AND i.indnatts = 1
            AND (SELECT attname FROM pg_attribute WHERE attrelid = i.indrelid AND attnum = i.indkey[0]) = 'pack_id') = 0
    AND (SELECT open_slot FROM public.beskt_method_versions WHERE id = b.rec_v) IS NULL
    AND (SELECT open_slot FROM public.beskt_method_versions WHERE pack_id = b.rec_pack AND version_number = 2) = 'open',
    'B3.50b the one-open-version rule is a unique index on (pack_id, open_slot), a generated slot that is NULL from published onward; pack_id alone is never unique (a method has many versions)');
  PERFORM set_config('beskt.governed_transition', 'on', true);
  PERFORM pg_temp.must_fail(
    format($q$INSERT INTO public.beskt_method_versions (pack_id, version_number, mode, source_reference, source_document_version, content_provenance)
...
BEGIN
  ...
  PERFORM pg_temp.must_fail_as('authenticated', 'b2000000-0000-4000-8000-0000000000f1',
    'SELECT public.beskt_holds_grant(''b2000000-0000-4000-8000-0000000000a1'', ''personnel_security'')',
    'permission denied', 'B5.18d a roleless user cannot ask who holds which gate');
  PERFORM pg_temp.must_fail_as('authenticated', 'b2000000-0000-4000-8000-0000000000c1',
    'SELECT public.beskt_holds_grant(''b2000000-0000-4000-8000-0000000000fa'', ''internal_qa'')',
    'permission denied', 'B5.18e a candidate cannot enumerate internal-QA grants');
  PERFORM pg_temp.must_fail_as('authenticated', 'b2000000-0000-4000-8000-0000000000d1',
    'SELECT public.beskt_reader_access_classes(''b2000000-0000-4000-8000-0000000000e1'')',
    'permission denied', 'B5.18f an employer member cannot read another user''s access classes');
  PERFORM pg_temp.must_fail_as('authenticated', 'b2000000-0000-4000-8000-0000000000e1',
    'SELECT public.beskt_reader_access_classes(''b2000000-0000-4000-8000-0000000000fa'')',
    'permission denied', 'B5.18g nor can an editor: the helpers are internal, their callers are SECURITY DEFINER');
  PERFORM pg_temp.must_fail_as('authenticated', 'b2000000-0000-4000-8000-0000000000e1',
...
BEGIN
  ...
  -- Direct writes are refused for every caller: a grant is made or revoked
  -- only inside the two governed RPCs, which set a transaction-local marker
  -- around their one write.
  PERFORM pg_temp.must_fail(
  ...
  PERFORM pg_temp.ok((SELECT revoked_at IS NULL FROM public.beskt_governance_grants WHERE id = _g2)
    AND (SELECT count(*) FROM public.beskt_governance_grants WHERE source_reference = 'fabricated') = 0,
    'B6.9f nothing was written by the refused direct writes');

  -- Expired and not-yet-valid grants, planted under the governed marker
  -- (the RPC always dates a grant from now).
  ...
  -- Append-only holds even inside a governed write: with the marker on, the
  -- only shape a write may take is one revocation.
  PERFORM set_config('beskt.governance_grant_write', 'on', true);
  PERFORM pg_temp.must_fail(format('UPDATE public.beskt_governance_grants SET user_id = ''b2000000-0000-4000-8000-0000000000f1'' WHERE id = %L', _g2),
    'BESKT_GRANT_APPEND_ONLY', 'B6.29 a grant cannot be re-assigned, even by the database owner under the governed marker');
  ...
  PERFORM set_config('beskt.governance_grant_write', 'off', true);
  PERFORM pg_temp.ok(
    (SELECT count(*) FROM pg_trigger t WHERE t.tgname = 'beskt_governance_grants_append_only'
       AND (t.tgtype & 4) = 4 AND (t.tgtype & 16) = 16 AND (t.tgtype & 8) = 8) = 1,
    'B6.31b the grant guard fires on INSERT, UPDATE and DELETE');
