-- TRUST Evidence Report — PR-R2A-1 (EXPAND): old reads and new entry points
-- work at the same time.
--
-- 20261024090000 adds scp_participant_report / scp_employer_report and removes
-- nothing. From the moment it is applied until PR-R2A-3 (CONTRACT) withdraws
-- the direct read, the hosted database serves BOTH the application on main
-- (direct RLS-bounded table reads) and the cutover code (the entry points).
-- This suite pins that:
--
--   E1  the code on main today still works, verbatim
--   E2  the code PR-R2A-2 ships already works
--   E3  nothing this step must not touch has moved
--
-- PR-R2A-3 reaches this state by rolling itself back and runs this file there.
--
-- The fixture is the report-audience suite's: the sg-operational-baseline
-- form walked by one candidate, reviewed, released. Own uuids (fe...). One
...
SELECT pg_temp.ok(
  ...
  'E1.3 and the row policy still hides the employer row from the participant');
RESET ROLE; RESET request.jwt.claim.sub;
...
SELECT pg_temp.must_fail(
  ...
  'permission denied', 'E2.4 anon cannot execute the employer entry point');
SELECT pg_temp.must_fail(
  format('SELECT count(*) FROM public.scp_participant_report(%L::uuid)', (SELECT attempt_id FROM run)),
  'permission denied', 'E2.5 nor the participant one');
RESET ROLE;

DO $$ BEGIN RAISE NOTICE 'GROUP E3 — nothing EXPAND must not touch has moved'; END $$;

SELECT pg_temp.ok(
  EXISTS (SELECT 1 FROM pg_policies WHERE schemaname = 'public'
           AND tablename = 'scp_competency_evidence' AND policyname = 'scp_evidence_own_select')
  AND (SELECT count(*) FROM pg_policies WHERE schemaname = 'public'
        AND tablename = 'scp_report_snapshots') = 2,
  'E3.1 the subject''s ledger policy and both snapshot row policies are exactly where main left them (R2A-3 changes them, not EXPAND)');
