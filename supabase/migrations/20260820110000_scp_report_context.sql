-- A released report says what it is about, and freezes that with the evidence.
--
-- ── WHAT WAS MISSING ────────────────────────────────────────────────────
--
-- The Phase 8 payloads carried competency lines and nothing else. A reader
-- could see eight competencies and their evidence states, but not which
-- assessment produced them, which version, for what purpose, under which
-- governance basis, when it was administered, or whether the human review was
-- finished. Source of Truth §9 Part A calls that Context and Integrity, and
-- without it the document is a fragment rather than a report.
--
-- The context is FROZEN at release rather than joined at render time. A report
-- is evidence: reading it in six months must show the assessment version and
-- validation status that applied on the day it was issued, not whatever the
-- catalogue says later.
--
-- ── WHY THE PARTICIPANT'S NAME IS NOT IN HERE ───────────────────────────
--
-- Two reasons, and both are deliberate.
--
-- First, erasure. A snapshot is immutable by trigger and cannot be deleted. A
-- name written into one could never be removed, which is exactly the situation
-- the pseudonymous subject model exists to avoid: scp_subjects holds no PII,
-- and identity lives in scp_subject_identities where it can be unlinked.
--
-- Second, the audited reveal. Identity resolution already runs through
-- scp_resolve_participant_identity, which requires owner/admin, checks the
-- attempt belongs to the organisation, and is an explicit act rather than an
-- ambient one. Freezing the name into the report would quietly replace a
-- deliberate, logged request with a name printed on every page view.
--
-- So the report carries the pseudonymous reference and the person CONTEXT
-- (employee or candidate), and the surface offers the same audited reveal it
-- always did. The employer gets the name when they ask for it.
--
-- ── ROLE, CUSTOMER AND SITE ─────────────────────────────────────────────
--
-- scp_attempts.role_version_id is nullable and is NULL for every attempt that
-- exists today; customer and site have no schema at all until Phase 9. Those
-- keys are therefore OMITTED from the context object rather than emitted as
-- null or "not provided". A field that is absent is honest; a field that says
-- "not provided" on every report is noise that trains the reader to skip the
-- section where it will one day matter.
--
-- Forward-only. Remediation: drop the column and restore the release function
-- from 20260820100000. Existing released snapshots are not read or rewritten;
-- their context is NULL and the surfaces degrade to what they showed before.

ALTER TABLE public.scp_report_snapshots
  ADD COLUMN IF NOT EXISTS context jsonb;

COMMENT ON COLUMN public.scp_report_snapshots.context IS
  'Audience-appropriate Part A context, frozen at release. NULL on rows released before this existed. Carries no participant name: identity stays behind the audited scp_resolve_participant_identity path.';
...
BEGIN
  ...
  -- Coverage, counted rather than assumed. The sufficiency gate is expressed in
  -- CONTEXTS, not in sittings: two attempts against the same form are still one
  -- evidence context, and a report that said "two occasions" while the gate saw
  -- one would explain the wrong thing.
  ...
  WITH lines AS (
    ...
    SELECT l.*,
           ...
           -- Which kinds of task contributed. Part C asks for the context
           -- types behind a line; this is the honest answer and is also what
           -- makes "one source" visible per competency rather than only in
           ...
           -- The observable behaviour the competency is read through. Part C
           -- asks what was actually looked at, not just the competency label.
  ...
  -- ── Part A, per audience ───────────────────────────────────────────────
  -- Role, customer and site are omitted rather than emitted as null: they do
  -- not exist in the data model yet, and an empty field on every report is
  ...
  -- The participant gets what concerns them: why, what, when, and under what
  -- limitation. Not the internal lifecycle, not the scoring model version, not
  -- review counts -- those describe how the machine ran, not their record.
