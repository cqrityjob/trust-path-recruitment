...
--
-- ADDITIVE ONLY (CREATE OR REPLACE of one function body).
--
-- ── THE DEFECT ─────────────────────────────────────────────────────────
--
-- scp_employer_assign generated the assignment's invitation_token_hash with:
--
--     encode(gen_random_bytes(24), 'hex')
--
-- gen_random_bytes() belongs to the pgcrypto EXTENSION. On Supabase, extensions
-- live in the `extensions` schema, and this function pins
-- `SET search_path = public, pg_temp` -- correctly, because an unpinned
-- search_path on a SECURITY DEFINER function is a privilege-escalation vector.
--
-- So the call could never resolve, and every assignment failed with
-- "function gen_random_bytes(integer) does not exist".
--
-- ── WHY THE SUITE MISSED IT ────────────────────────────────────────────
--
-- The CI harness is a bare Postgres with pgcrypto present and reachable, and
-- the SQL journey tests build their assignment rows with a literal token hash
-- rather than calling scp_employer_assign. Nothing exercised this line against
-- a Supabase-shaped database until the browser did.
--
-- ── THE FIX ────────────────────────────────────────────────────────────
--
-- Schema-qualifying extensions.gen_random_bytes() would work, but it couples a
-- core assignment path to an extension being installed in a particular schema.
-- gen_random_uuid() is core Postgres (13+) and already used throughout this
-- schema for primary keys, so the dependency is simply removed.
--
-- The value is a uniqueness token, not a secret: the Academy flow authenticates
-- participants through their own account rather than through a link, so this
-- column exists to satisfy the legacy NOT NULL UNIQUE constraint. Two uuids
-- hashed together give far more collision resistance than that needs.
...
BEGIN
  ...
  VALUES
    (_employer_id, _assessment_version_id, 'academy', 'workforce', _email,
     _user, auth.uid(),
     -- Core Postgres only. See the header: pgcrypto is not reachable from this
     -- function's pinned search_path, and pinning it is not negotiable.
...
BEGIN
  ...
  -- The search_path must stay pinned. Removing it would "fix" the original
  -- error for the wrong reason and open a SECURITY DEFINER escalation path.
