-- Security Career Discovery v3.1 — PR 2: instrument definition and option matrix.
--
-- ADDITIVE ONLY. Registers v3.1 as a NEW definition version alongside v3.0,
-- seeds its 20-item registry, and seeds the 164 option-level dimension
-- loadings that back the eight single-choice items.
--
-- v3.0 is NOT retired here. It keeps its own definition version, its own 42
-- registry items and its own snapshots, and stays exactly as it is. The
-- cutover — retiring v3.0 and pointing the routes at v3.1 — is a later PR,
-- so this one cannot break a working product and can be reverted by deleting
-- three sets of rows.
--
-- Every value seeded here is generated from
-- src/lib/career-discovery/v31/option-matrix.ts. scripts/career-discovery-v31-check.ts
-- re-parses this file and asserts the two are identical, tuple for tuple, so
...
SELECT
  ...
  '2026-scd-v3.1.0',
  'v1',
  'Security Career Discovery v3.1 — 16 Career Intelligence Dimensions, 20 core '
  'scored items (12 scale + 8 single choice), option-level loadings and the ten '
  'Career Patterns. NOT ACTIVE: lifecycle status is internal_test and the content '
  'review gates are outstanding. See src/lib/career-discovery/v31/.'
WHERE NOT EXISTS (
...
-- =========================================================================
-- 2. The 20-item registry
-- =========================================================================
--
-- Section assignment is four items per section in display order. The five
-- Discovery sections are inherited unchanged from v3.0; nothing in the
-- scoring model depends on which section an item sits in, so this is a
-- presentation grouping and is marked provisional until the session flow
-- lands in a later PR.
...
-- =========================================================================
-- 3. The option matrix — 164 loadings across 32 options
-- =========================================================================
--
-- Generated from option-matrix.ts. Do not hand-edit: the guard script
-- compares this block against the TypeScript source and will fail on any
-- difference, including whitespace inside a rationale.
...
-- apply if the counts or the set-level invariants are wrong, so a partial or
-- corrupted seed can never be mistaken for a good one.
...
BEGIN
  ...
  -- The Delivery A set-level invariants, checked on the real seeded data:
