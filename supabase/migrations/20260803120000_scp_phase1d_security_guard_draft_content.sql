...
--
-- ADDITIVE ONLY. Every row created here is `draft`. Nothing is published,
-- nothing is assignable, nothing becomes employer-visible, and no assessment
-- definition is activated. Publication is a separate, owner-approved step that
-- requires expert, legal and cognitive review.
--
-- ── THE 8 COMPETENCE DIMENSIONS ────────────────────────────────────────
--
-- The brief lists eight dimensions for this role. They are modelled as
-- OBSERVABLE BEHAVIOURS mapped onto the existing SCC-01..SCC-12 competencies,
-- NOT as a second competency taxonomy. That is the whole point of the behaviour
-- layer: role-specific vocabulary attaches to a stable competency spine, so the
-- graph serves the entire security industry rather than one role.
--
-- ── WHAT THIS CONTENT DOES NOT CLAIM ───────────────────────────────────
--
-- Nothing here measures personality, honesty as a trait, emotional stability,
-- real workplace performance, stress tolerance, motivation, future job
-- performance, physical capability, formal authorisation or legal eligibility.
-- The programme states this in does_not_measure_sv/en, which the publication
-- guard requires.

-- =========================================================================
-- SECTION 0 — A Phase 0 oversight, corrected
-- =========================================================================
--
-- Phase 0 widened scp_assessment_definitions.purpose to include
-- 'development_programme', but did NOT widen its companion constraint
-- scp_definition_profession_matches_purpose, which enumerates only the two
-- original purposes. The new purpose therefore matched neither branch and was
-- rejected outright.
--
-- Found by this migration failing, not by review. Corrected here rather than by
-- editing the applied Phase 0 file.
--
-- For a development programme the profession is OPTIONAL: role specificity is
-- expressed through scp_programs.role_id and scp_role_versions, which carry the
-- jurisdiction too. Requiring profession_id as well would duplicate that.
...
-- =========================================================================
-- SECTION 1 — Role, programme, modules
-- =========================================================================
...
-- =========================================================================
-- SECTION 2 — The 8 dimensions as observable behaviours
-- =========================================================================
--
-- Each maps to an existing SCC competency. Behaviours describe what a person
-- visibly DOES, never a trait.
...
-- Behaviour → competency. The 8 role dimensions attach to the stable spine.
...
-- Role → competency, so the role's profile is traversable.
...
-- Module → behaviour, closing the development loop.
...
-- =========================================================================
-- SECTION 3 — The 18-item baseline form (assessment mode, DRAFT)
-- =========================================================================
--
-- 12 single-best-response SJT + 3 best/worst + 3 constructed response.
-- Each item has ONE primary behaviour, which the agreement guard checks against
-- the competency the item claims.
...
-- The items. Bilingual stem text lives in scp_item_texts; option labels in
-- scp_item_option_texts; option scores and rationales stay authoring-only.
INSERT INTO public.scp_items (slug)
SELECT 'sg-b-' || lpad(g::text, 2, '0') FROM generate_series(1, 18) g
ON CONFLICT (slug) DO NOTHING;

-- observable_behavior (PR-A, NOT NULL text) is the FREE-TEXT ancestor of what
-- Phase 0 made structural. Both are populated from the same behaviour version,
-- so the prose and the graph link cannot disagree.
...
BEGIN
  ...
  -- Every dimension reaches a competency: no orphan behaviour.
  SELECT count(*) INTO _n FROM public.scp_behaviour_versions bv
    JOIN public.scp_observable_behaviours b ON b.id = bv.behaviour_id
   WHERE b.slug IN ('situational_judgement','proportional_decision_making',
     'mandate_and_escalation','operational_communication','de_escalation',
  ...
  -- Every item maps to exactly one behaviour and is assessment-mode.
  SELECT count(*) INTO _n FROM public.scp_item_versions iv
    JOIN public.scp_items i ON i.id = iv.item_id
  ...
  -- DRAFT ONLY. Nothing published, nothing assignable, nothing employer-visible.
