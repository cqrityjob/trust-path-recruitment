-- ############################################################################
-- PR-R3B COPY DECISIONS -- competency display label, and plural generation.
--
-- Two Product Owner decisions, and nothing else. No scoring rule, no
-- threshold, no evidence rule, no audience boundary and no report contract is
-- touched, and no released report is rewritten.
--
-- ── DECISION 1: SCC-07's Swedish display label ─────────────────────────────
--
--   "Respektfull service och gränshållning"
--     -> "Professionellt bemötande och gränssättning"
--
-- The label is normative content, so a published competency version cannot be
-- edited: scp_guard_published_immutable allows only lifecycle columns to
-- change on a published row, which is exactly the protection that makes a
-- released report readable years later. The label therefore moves the way
-- every other content correction moves -- by publishing a NEW version and
-- retiring the old one.
--
-- What is preserved, and asserted below: the competency code (SCC-07), the
-- definition in both languages verbatim, every indicator array, the
-- interpretation rules, and the facets (which hang off the competency, not
-- the version). Version 2 differs from version 1 in two strings only.
--
-- The English label moves with it, for parity: "Service Orientation" no
-- longer describes a label about professional conduct AND boundary setting,
-- and the house style for these names is "X & Y". The DEFINITION is
-- unchanged, so nothing about what the competency means has moved.
--
-- HISTORICAL REPORTS DO NOT CHANGE. A released report is a frozen document
-- that carries its own copy of the competency name, and the audience contract
-- projects that copy. The one path that resolves a name live -- the V3
-- projection's fallback, for a competency with no frozen line -- is already
-- time-aware: it considers only versions created at or before the report's
-- released_at, so a report released before this migration can never see
-- version 2. Section 3 asserts both, and section 4 proves that not one byte
-- of any released snapshot changed while this migration ran.
--
-- ── DECISION 2: the generator writes grammatical Swedish and English ───────
--
-- scp_release_attempt_report wrote one sentence for both counts:
--
--   "Endast 1 uppgift(er) i den här bedömningen berörde området ..."
--   "Only 1 task(s) in this assessment touched this area ..."
--
-- A placeholder like that reads as unfinished software in a document a
-- recruitment manager takes into an interview. The function now agrees the
-- noun with the number in every branch of the observed-line copy, in both
-- languages. For any count other than 1 the sentence is byte-identical to
-- what it produced before, so nothing that has already been released could
-- have been written differently.
--
-- The client keeps a compatibility repair for reports that were released
-- BEFORE this migration and still carry the placeholder in their frozen text
-- (src/lib/security-competency/trust-report.types.ts, repairPlurals). That is
-- legacy compatibility, not the generation path: from here the database emits
-- correct strings and the repair has nothing to do.
--
-- Refuses to apply unless PR-R1 and PR-R3A are already applied and SCC-07 is
-- in the state this migration expects.
-- ############################################################################

-- ============================================================================
-- SECTION 0 -- Preconditions, and a fingerprint of every released report
-- ============================================================================

DO $$
DECLARE _n int;
BEGIN
  IF to_regprocedure('public.scp_employer_report_v3(uuid)') IS NULL THEN
    RAISE EXCEPTION 'SCP_R3B_COPY_PRECONDITION: PR-R3A (20261029090000) must be applied first';
  END IF;
  IF to_regprocedure('public.scp_release_attempt_report(uuid)') IS NULL THEN
    RAISE EXCEPTION 'SCP_R3B_COPY_PRECONDITION: scp_release_attempt_report is missing';
  END IF;
  ...
  SELECT count(*) INTO _n
   ...
   WHERE c.code = 'SCC-07';
  IF _n <> 1 THEN
    RAISE EXCEPTION 'SCP_R3B_COPY_PRECONDITION: expected exactly one SCC-07 version, found %', _n;
  END IF;
  ...
  SELECT count(*) INTO _n
   ...
   WHERE c.code = 'SCC-07'
     ...
     AND v.name_sv = 'Respektfull service och gränshållning';
  IF _n <> 1 THEN
    RAISE EXCEPTION 'SCP_R3B_COPY_PRECONDITION: SCC-07 v1 is not the published row this migration expects';
  END IF;
END $$;

-- The fingerprint. Every released snapshot, its payload, its brief and its
-- context, hashed as one value before anything below runs. Section 4 takes
-- the same fingerprint afterwards and refuses to finish if it moved.
-- Session-scoped, not ON COMMIT DROP: a migration may be applied statement by
-- statement outside one transaction, and the fingerprint has to outlive that.
DROP TABLE IF EXISTS _r3b_frozen_before;
CREATE TEMP TABLE _r3b_frozen_before AS
SELECT
  count(*) AS snapshots,
  coalesce(
    md5(string_agg(
      id::text || coalesce(payload::text, '') || coalesce(brief::text, '') ||
      coalesce(context::text, '') || released_at::text,
      '|' ORDER BY id)),
    'no-rows') AS digest
FROM public.scp_report_snapshots;

-- ============================================================================
-- SECTION 1 -- DECISION 1: SCC-07 display label, as a new published version
-- ============================================================================

-- Enters the world as a draft, because scp_guard_version_starts_as_draft
-- refuses anything else -- including from a migration. Every normative field
-- is copied from version 1; only the two display labels differ.
INSERT INTO public.scp_competency_versions (
  competency_id, version_number, content_status,
  name_sv, name_en, definition_sv, definition_en,
  strong_indicators_sv, risk_indicators_sv, development_indicators_sv,
  does_not_measure_sv, interpretation_rule_sv, interpretation_rule_en)
SELECT
  v.competency_id, 2, 'draft',
  'Professionellt bemötande och gränssättning',
  'Professional Conduct & Boundary Setting',
  v.definition_sv, v.definition_en,
  v.strong_indicators_sv, v.risk_indicators_sv, v.development_indicators_sv,
  v.does_not_measure_sv, v.interpretation_rule_sv, v.interpretation_rule_en
FROM public.scp_competency_versions v
JOIN public.scp_competencies c ON c.id = v.competency_id
WHERE c.code = 'SCC-07' AND v.version_number = 1;

UPDATE public.scp_competency_versions v
   SET content_status = 'published', published_at = now()
  FROM public.scp_competencies c
 WHERE c.id = v.competency_id AND c.code = 'SCC-07' AND v.version_number = 2;

-- One live wording per competency. Version 1 stays readable forever -- every
-- report released against it points at this row -- but it is no longer the
-- version new content is authored or resolved against.
UPDATE public.scp_competency_versions v
   SET content_status = 'retired', retired_at = now()
  FROM public.scp_competencies c
 WHERE c.id = v.competency_id AND c.code = 'SCC-07' AND v.version_number = 1;

-- The maps move with the wording, so a report released from here resolves the
-- new label. Historical evidence rows are untouched: they reference behaviour
-- versions, and their frozen reports carry their own names.
UPDATE public.scp_behaviour_competency_map m
   SET competency_version_id = v2.id
  FROM public.scp_competency_versions v1
  JOIN public.scp_competencies c ON c.id = v1.competency_id
  JOIN public.scp_competency_versions v2
    ON v2.competency_id = v1.competency_id AND v2.version_number = 2
 WHERE c.code = 'SCC-07' AND v1.version_number = 1
   AND m.competency_version_id = v1.id;

UPDATE public.scp_role_competency_map m
   SET competency_version_id = v2.id
  FROM public.scp_competency_versions v1
  JOIN public.scp_competencies c ON c.id = v1.competency_id
  JOIN public.scp_competency_versions v2
...
UPDATE public.scp_interview_pack_competency_map m
   SET competency_version_id = v2.id
  FROM public.scp_competency_versions v1
  JOIN public.scp_competencies c ON c.id = v1.competency_id
  JOIN public.scp_competency_versions v2
...
-- ============================================================================
-- SECTION 2 -- DECISION 2: the generator agrees the noun with the number
--
-- Byte-for-byte the definition applied by PR-R1 (20261027090000), with the
-- ten observed-line sentences made count-aware. Nothing else in the function
-- changes: the same scope, the same manifest, the same evidence rules.
-- ============================================================================
    ...
    FROM (
      SELECT jsonb_build_object(
               ...
               'why_sv', CASE sig.signal
                 WHEN 'strong'     THEN format('Svaren höll en jämn och hög nivå över %s %s i den här bedömningen.', sig.observations, CASE WHEN sig.observations = 1 THEN 'uppgift' ELSE 'uppgifter' END)
                 WHEN 'consistent' THEN format('Svaren pekade åt samma håll över %s %s i den här bedömningen.', sig.observations, CASE WHEN sig.observations = 1 THEN 'uppgift' ELSE 'uppgifter' END)
                 WHEN 'mixed'      THEN format('Svaren skilde sig åt mellan jämförbara uppgifter (%s %s, spännvidd %s).', sig.observations, CASE WHEN sig.observations = 1 THEN 'uppgift' ELSE 'uppgifter' END, to_char(sig.spread,'FM0.00'))
                 WHEN 'developing' THEN format('Svaren valde genomgående handlingsalternativ som uppgifterna beskriver som mindre välavvägda (%s %s).', sig.observations, CASE WHEN sig.observations = 1 THEN 'uppgift' ELSE 'uppgifter' END)
                 ELSE format('Endast %s %s i den här bedömningen berörde området — för lite för att säga något om det.', sig.observations, CASE WHEN sig.observations = 1 THEN 'uppgift' ELSE 'uppgifter' END)
               END,
               'why_en', CASE sig.signal
                 WHEN 'strong'     THEN format('Answers were consistently strong across %s %s in this assessment.', sig.observations, CASE WHEN sig.observations = 1 THEN 'task' ELSE 'tasks' END)
                 WHEN 'consistent' THEN format('Answers pointed the same way across %s %s in this assessment.', sig.observations, CASE WHEN sig.observations = 1 THEN 'task' ELSE 'tasks' END)
                 WHEN 'mixed'      THEN format('Answers differed between comparable tasks (%s %s, spread %s).', sig.observations, CASE WHEN sig.observations = 1 THEN 'task' ELSE 'tasks' END, to_char(sig.spread,'FM0.00'))
                 WHEN 'developing' THEN format('Answers consistently chose options the tasks describe as less well-judged (%s %s).', sig.observations, CASE WHEN sig.observations = 1 THEN 'task' ELSE 'tasks' END)
                 ELSE format('Only %s %s in this assessment touched this area — too few to say anything about it.', sig.observations, CASE WHEN sig.observations = 1 THEN 'task' ELSE 'tasks' END)
               END
             ) AS x
...
COMMENT ON FUNCTION public.scp_release_attempt_report(uuid) IS
  ...
  'transaction under one calculated_at. PR-R3B: the observed-line copy agrees '
  'the noun with the number in both languages, so no report written from here '
  'can contain "uppgift(er)" or "task(s)". For any count other than one the '
  'sentence is identical to what PR-R1 produced, so no already-released report '
  'would have been written differently.';

-- ============================================================================
-- SECTION 3 -- PROOF: what changed, and what could not have
-- ============================================================================

DO $$
DECLARE
  _v1 public.scp_competency_versions%ROWTYPE;
  _v2 public.scp_competency_versions%ROWTYPE;
  _src text;
  _n int;
BEGIN
  SELECT v.* INTO _v1 FROM public.scp_competency_versions v
    JOIN public.scp_competencies c ON c.id = v.competency_id
   WHERE c.code = 'SCC-07' AND v.version_number = 1;
  SELECT v.* INTO _v2 FROM public.scp_competency_versions v
    JOIN public.scp_competencies c ON c.id = v.competency_id
   WHERE c.code = 'SCC-07' AND v.version_number = 2;

  -- 3.1 The label moved, in both languages.
  IF _v2.name_sv <> 'Professionellt bemötande och gränssättning'
     OR _v2.name_en <> 'Professional Conduct & Boundary Setting' THEN
    RAISE EXCEPTION 'SCP_R3B_COPY_PROOF 3.1: version 2 does not carry the approved labels';
  END IF;

  -- 3.2 Nothing normative moved with it.
  IF _v2.definition_sv IS DISTINCT FROM _v1.definition_sv
     OR _v2.definition_en IS DISTINCT FROM _v1.definition_en
     OR _v2.strong_indicators_sv IS DISTINCT FROM _v1.strong_indicators_sv
     OR _v2.risk_indicators_sv IS DISTINCT FROM _v1.risk_indicators_sv
     OR _v2.development_indicators_sv IS DISTINCT FROM _v1.development_indicators_sv
     OR _v2.does_not_measure_sv IS DISTINCT FROM _v1.does_not_measure_sv
     OR _v2.interpretation_rule_sv IS DISTINCT FROM _v1.interpretation_rule_sv
     OR _v2.interpretation_rule_en IS DISTINCT FROM _v1.interpretation_rule_en
     OR _v2.competency_id <> _v1.competency_id THEN
    RAISE EXCEPTION 'SCP_R3B_COPY_PROOF 3.2: version 2 changed more than the display label';
  END IF;

  -- 3.3 Version 1 still says what it always said, and is still readable.
  IF _v1.name_sv <> 'Respektfull service och gränshållning'
     OR _v1.name_en <> 'Service Orientation'
     OR _v1.content_status <> 'retired' THEN
    RAISE EXCEPTION 'SCP_R3B_COPY_PROOF 3.3: version 1 is not the retired historical wording';
  END IF;

  -- 3.4 Exactly one published version per competency, everywhere.
  SELECT count(*) INTO _n FROM (
    SELECT competency_id FROM public.scp_competency_versions
     WHERE content_status = 'published'
     GROUP BY competency_id HAVING count(*) > 1) d;
  IF _n > 0 THEN
    RAISE EXCEPTION 'SCP_R3B_COPY_PROOF 3.4: % competency(ies) now have more than one published version', _n;
  END IF;

  -- 3.5 Every map for SCC-07 points at the new version, and none was lost.
  SELECT count(*) INTO _n FROM public.scp_behaviour_competency_map m
   WHERE m.competency_version_id = _v1.id;
  IF _n > 0 THEN RAISE EXCEPTION 'SCP_R3B_COPY_PROOF 3.5: % behaviour map row(s) left on version 1', _n; END IF;
  SELECT count(*) INTO _n FROM public.scp_role_competency_map m
   WHERE m.competency_version_id = _v1.id;
  IF _n > 0 THEN RAISE EXCEPTION 'SCP_R3B_COPY_PROOF 3.5: % role map row(s) left on version 1', _n; END IF;
  SELECT count(*) INTO _n FROM public.scp_interview_pack_competency_map m
   WHERE m.competency_version_id = _v1.id;
  IF _n > 0 THEN RAISE EXCEPTION 'SCP_R3B_COPY_PROOF 3.5: % pack map row(s) left on version 1', _n; END IF;

  -- 3.6 The facets are attached to the competency, not to a version, so the
  --     label change cannot have orphaned one.
  SELECT count(*) INTO _n FROM public.scp_competency_facets f
   WHERE f.competency_id = _v1.competency_id;
  IF _n = 0 THEN
    RAISE EXCEPTION 'SCP_R3B_COPY_PROOF 3.6: SCC-07 lost its facets';
  END IF;

  -- 3.7 The generator cannot write a placeholder any more.
  _src := pg_get_functiondef('public.scp_release_attempt_report(uuid)'::regprocedure);
  IF _src LIKE '%uppgift(er)%' OR _src LIKE '%task(s)%' THEN
    RAISE EXCEPTION 'SCP_R3B_COPY_PROOF 3.7: the release function still emits a plural placeholder';
  END IF;
  IF position('THEN ''uppgift'' ELSE ''uppgifter''' in _src) = 0
     OR position('THEN ''task'' ELSE ''tasks''' in _src) = 0 THEN
    RAISE EXCEPTION 'SCP_R3B_COPY_PROOF 3.7: the release function is not count-aware in both languages';
  END IF;

  RAISE NOTICE 'ok  PR-R3B copy decisions: SCC-07 label published as version 2, generator is count-aware';
END $$;

-- ============================================================================
-- SECTION 4 -- PROOF: not one released report was touched
-- ============================================================================

DO $$
DECLARE _before record; _after record;
BEGIN
  SELECT * INTO _before FROM _r3b_frozen_before;
  SELECT
    count(*) AS snapshots,
    coalesce(
      md5(string_agg(
  ...
  IF _before.snapshots <> _after.snapshots OR _before.digest <> _after.digest THEN
    RAISE EXCEPTION
      'SCP_R3B_COPY_PROOF 4: a released report changed while this migration ran (% -> % snapshots)',
      _before.snapshots, _after.snapshots;
  END IF;
  RAISE NOTICE 'ok  PR-R3B copy decisions: % released report(s) byte-identical', _after.snapshots;
END $$;
