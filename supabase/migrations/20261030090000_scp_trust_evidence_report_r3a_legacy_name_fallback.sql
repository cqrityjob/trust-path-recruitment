...
--
-- ONE defect, ONE change. scp_employer_report_v3 (20261029090000) resolves a
-- competency's name from the frozen document, and falls back to the catalogue
-- version that was published at the release instant only when the document
-- has no name for it. The fallback was written as
--
--     SELECT coalesce(_name_sv, cv.name_sv), ... INTO _name_sv, _name_en, _cver
--
-- and a SELECT INTO that finds no row assigns NULL. On the production project
-- the competency catalogue was recreated by the 2026-08-28 bootstrap, so every
-- report released before that date has catalogue rows CREATED AFTER its own
-- released_at; the lookup found nothing and overwrote the names the frozen
-- brief and payload had already supplied. Every hosted employer document came
-- back with eight nameless competencies (observed 2026-09-06 immediately after
-- the R3A apply, ledger 20260906125945). The local suites could not see it:
-- a local catalogue always predates a local release.
--
-- The fix lands the lookup in its own variables and fills only what is
-- empty. A manifest-backed report never entered the fallback for its name;
-- a legacy report keeps the frozen document's names and its explicit legacy
-- null for competency_version; a report whose catalogue does predate it
-- resolves exactly as before. supabase/tests/scp_trust_evidence_report_r3a_
-- legacy_name_fallback_test.sql reproduces the production condition and
-- proves all three.
--
-- Everything else in the function is byte-for-byte the PR-R3A definition, and
-- the PR-R3A apply-time proof runs again at the end of this file. Privileges
-- survive CREATE OR REPLACE; nothing else is touched. No row is written.
--
-- Rollback: supabase/rollback/20261030090000_scp_trust_evidence_report_r3a_legacy_name_fallback_rollback.sql
-- =============================================================================
...
BEGIN
  IF to_regprocedure('public.scp_employer_report_v3(uuid)') IS NULL THEN
    RAISE EXCEPTION 'SCP_R3A_FALLBACK_PRECONDITION: scp_employer_report_v3 is missing -- apply 20261029090000 (PR-R3A) first';
  END IF;
  SELECT p.prosrc INTO _src FROM pg_proc p WHERE p.oid = 'public.scp_employer_report_v3(uuid)'::regprocedure;
  IF _src LIKE '%coalesce(_name_sv, _fb_sv)%' THEN
    RAISE NOTICE 'PR-R3A fallback hotfix: already in place';
  ELSIF _src NOT LIKE '%INTO _name_sv, _name_en, _cver%' THEN
    RAISE EXCEPTION 'SCP_R3A_FALLBACK_PRECONDITION: scp_employer_report_v3 is neither the PR-R3A definition nor this hotfix';
  END IF;
END
$pre$;
...
BEGIN
  ...
  FOR _c IN SELECT x FROM jsonb_array_elements_text(_codes) x LOOP
    ...
    IF _name_sv IS NULL OR _cver IS NULL THEN
      -- A lookup that finds no row must take nothing away: SELECT INTO
      -- assigns NULL on zero rows, and on a report whose catalogue rows were
      -- created after its release (every report released before a project
      -- bootstrap) that erased the names the frozen document had supplied.
      -- The lookup lands in its own variables and only fills what is empty.
      _fb_sv := NULL; _fb_en := NULL; _fb_ver := NULL;
      SELECT cv.name_sv, cv.name_en, cv.version_number::text
        INTO _fb_sv, _fb_en, _fb_ver
       ...
       LIMIT 1;
      _name_sv := coalesce(_name_sv, _fb_sv);
      _name_en := coalesce(_name_en, _fb_en);
      _cver    := coalesce(_cver, _fb_ver);
    END IF;
...
-- ═══════════════════════════════════════════════════════════════════════════
-- Proof, at apply time: the fallback is the only change, and the PR-R3A
-- contract still holds in full.
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  SELECT p.prosrc INTO _src FROM pg_proc p WHERE p.oid = 'public.scp_employer_report_v3(uuid)'::regprocedure;
  IF _src NOT LIKE '%coalesce(_name_sv, _fb_sv)%' OR _src NOT LIKE '%coalesce(_cver, _fb_ver)%'
     OR _src LIKE '%INTO _name_sv, _name_en, _cver%' THEN
    RAISE EXCEPTION 'SCP_R3A_FALLBACK_PROOF: the fallback was not replaced';
  END IF;
  IF has_function_privilege('anon', 'public.scp_employer_report_v3(uuid)'::regprocedure, 'EXECUTE')
    ...
    RAISE EXCEPTION 'SCP_R3A_FALLBACK_PROOF: the V3 contract grants moved';
  END IF;
  RAISE NOTICE 'PR-R3A fallback hotfix proven: a lookup that finds nothing takes nothing away';
END
$fix$;
