...
-- ── THE DEFECT ─────────────────────────────────────────────────────────
--
-- scp_submit_attempt scored whatever responses happened to exist. A run with
-- three of four items answered submitted cleanly, wrote evidence for the three,
-- and reached 'scored'.
--
-- Observed in local staging: the constructed response failed to save, the
-- participant submitted, and the attempt went straight to 'scored' with
-- reviews_opened = 0. The one item that REQUIRES a human was simply absent, so
-- no review was ever created and the report became releasable with no person
-- having looked at anything.
--
-- That is the serious part. The human-review gate is not bypassed by defeating
-- it -- it is bypassed by the response never existing. A missing answer removed
-- the requirement instead of blocking the result.
--
-- ── WHY EVERY TEST MISSED IT ───────────────────────────────────────────
--
-- Every test answers every item before submitting, because that is the case
-- worth asserting. Nothing submitted a partial run, so nothing noticed that
-- partial runs were accepted.
--
-- ── THE FIX ────────────────────────────────────────────────────────────
--
-- Submission requires a response for every item on the attempt's form. This is
-- the same reasoning as Phase 2k: a run may be incomplete while it is open --
-- that is what "in progress" means -- and must be complete at the boundary
-- where answers become evidence.
--
-- The two gates together now say: everything answered, and every best/worst
-- answer whole.
