-- Security Competence Academy — Phase 1a: programmes, modules, scenarios,
-- and the Learning / Assessment mode separation.
--
-- ADDITIVE ONLY. Extends the existing scp_* item bank rather than building a
-- parallel assessment model. No existing row is modified, no table dropped, no
-- policy weakened, no applied migration edited.
--
-- ── WHAT THIS ADDS ─────────────────────────────────────────────────────
--
--   Programme → Module → Scenario → Item
--
-- attached to the Phase 0 graph, which already runs
--
--   Role → Competency → Observable Behaviour → Item
--
-- The item bank (scp_items, scp_item_versions, scp_item_options,
-- scp_item_texts, scp_forms, scp_form_items) is REUSED unchanged in shape and
-- extended with three columns.
--
-- ── THE ONE OVERLAP, AND HOW IT IS RESOLVED ────────────────────────────
--
-- scp_item_versions already carries competency_id NOT NULL from PR-A, while the
-- approved graph says an item maps to exactly ONE observable behaviour and the
-- behaviour maps to competencies. That is two sources of truth for the same
-- fact.
--
-- Dropping competency_id would break PR-A's own tests and its NOT NULL
-- contract. Instead primary_behaviour_id becomes the AUTHORITATIVE graph link,
-- and a trigger requires the two to AGREE: the behaviour must map to the
-- competency the item claims. They can therefore never drift apart, and a later
-- migration can retire competency_id safely once nothing reads it.

-- =========================================================================
-- SECTION 1 — Programmes, modules, scenarios
-- =========================================================================
...
CREATE TABLE IF NOT EXISTS public.scp_program_versions (
  ...
  -- What this programme explicitly does NOT measure. Authored, surfaced in the
  -- report, and required before publication -- see the guard below.
...
-- A module develops specific behaviours. This is what turns a competency gap
-- into a learning recommendation without any module reading assessment items.
...
CREATE TABLE IF NOT EXISTS public.scp_scenario_versions (
  ...
  content_status text NOT NULL DEFAULT 'draft'
    CHECK (content_status IN (
      'draft','expert_review','legal_review','cognitive_review',
      'published','suspended','retired')),
  -- A scenario belongs to exactly one mode. The same situation may exist in
  -- both, but as two separate scenarios with separate content -- never shared.
...
-- A programme may not be published without saying what it does not measure.
-- The honesty requirement is a release gate, not a documentation convention.
...
-- =========================================================================
-- SECTION 2 — Item bank extensions
-- =========================================================================
--
-- Three columns on the EXISTING scp_item_versions. No parallel item model.

-- 2a. The graph link. Nullable so PR-A's existing rows stay valid; required for
--     any item that is published under the Academy (guarded below).
...
-- 2b. Mode. The same exposed question must never serve both open training and
...
-- 2c. The 18-item baseline needs two formats PR-A did not have.
ALTER TABLE public.scp_item_versions
...
-- The item bank's content_status must also carry the three review gates.
ALTER TABLE public.scp_item_versions
...
-- -------------------------------------------------------------------------
-- 2d. The agreement guard -- the resolution of the two-sources-of-truth problem
-- -------------------------------------------------------------------------
...
BEGIN
  IF NEW.primary_behaviour_id IS NULL THEN
    -- A PR-A item that predates the graph. It may not be published under the
    -- Academy, which the form-composition guard enforces at assembly time.
    RETURN NEW;
  END IF;
...
-- -------------------------------------------------------------------------
-- 2e. Mode disjointness
-- -------------------------------------------------------------------------
--
-- Two rules, because either alone leaves the boundary open:
--   * an item's mode is immutable once set -- otherwise an exposed training
--     item could be silently promoted into the controlled bank;
...
-- NOTE ON SCENARIO TEXT: scp_scenario_versions is readable because a learning
-- scenario is meant to be read. Assessment-mode scenario text is protected by
-- the ASSESSMENT item bank's own restrictions -- scp_item_texts and
-- scp_item_options are already authoring-only from PR-A, and delivery happens
-- through a server function in Phase 3. No answer key, option score or rationale
...
BEGIN
  ...
  -- The item bank gained its three columns, and no existing row was disturbed.
  ...
  -- Nothing was published or made assignable by this migration.
