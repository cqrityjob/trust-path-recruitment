...
-- for that recruitment. Every rec_* function enforces it for itself.
--
...
-- application.
--
-- ── THIS IS THE EXPAND HALF ────────────────────────────────────────────────
--
-- The rules above also need a BACKSTOP on job_applications itself -- a
-- decision trigger that stops a hand-made call to set_application_status(),
-- and a deferred trigger that stops an application inserted without its
-- required answers. Those two triggers are NOT here. They are
-- 20261208090000_recruitment_workspace_backstops.sql, the CONTRACT half,
-- because they refuse things TODAY'S application does: its decision buttons
-- call set_application_status() for every member, and its apply dialog sends
-- no answers. Applied before the new application is live, they would turn a
-- plain member's decision into "Kunde inte uppdatera ansökans status." with no
-- explanation; left in place after the application is rolled back, the same.
-- So: this file first (safe under today's code), the application next, the
-- backstops with it, and on an application rollback the backstops go first.
-- docs/release/2026-09-23-recruitment-workspace.md has the order.
...
-- 5b / 5c. The two backstops on job_applications -- required answers checked
-- at COMMIT whatever path created the row, and decisions refused to anyone who
-- may not make them whatever path moved the row -- are the CONTRACT half,
-- 20261208090000_recruitment_workspace_backstops.sql. See the header for why.
-- Until then rec_submit_application and rec_set_application_stage enforce the
-- same two rules themselves, so the new application is never without them.
...
DECLARE
  ...
  _job_id  uuid;
BEGIN
  SELECT a.status, a.job_id INTO _current, _job_id FROM public.job_applications a
  ...
  END IF;
  -- The decision rule, held here for the new application's own path. The
  -- trigger in 20261208090000 holds the same rule for every OTHER path.
  IF EXISTS (SELECT 1 FROM public.recruitment_settings s
              WHERE s.job_id = _job_id AND s.completion_state <> 'open') THEN
    RAISE EXCEPTION 'RECRUITMENT_COMPLETED' USING ERRCODE = 'check_violation';
  END IF;
  IF _new_status IN ('hired', 'rejected') AND NOT public.rec_can_manage(_job_id) THEN
