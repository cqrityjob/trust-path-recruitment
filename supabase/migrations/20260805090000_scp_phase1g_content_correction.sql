...
--
-- Three defects from Phase 1F, corrected:
--   1. Candidate-facing option labels reused the internal scoring rationale.
--   2. sg-b-02 encoded a coercive ID power a Swedish guard does not generally hold.
--   3. Items carried no primary-construct classification, so an item needing
--      specialist legal knowledge was indistinguishable from an SJT.
...
ALTER TABLE public.scp_item_versions
  ...
  -- The legal or operational proposition the item rests on, written out so a
  -- reviewer can accept or reject it rather than having to infer it.
  ...
  -- What the item actually tests. Guards against claiming an SJT measures law.
...
-- An item that cannot be answered from the scenario alone must not masquerade
-- as situational judgement.
...
-- =========================================================================
--
-- The Phase 1F version scored option C ("demand ID and prevent the person from
-- moving on") as 0 with the rationale that a guard "has no general right to
-- demand ID from a resident". That framing invented a legal proposition to knock
-- it down, and made the item turn on statutory power rather than judgement.
--
-- Rewritten so the preferred response rests on the ACCESS-CONTROL CONDITION of
-- the assignment, not on a coercive power: explain the condition, request
-- voluntary verification where the assignment supports it, and escalate through
-- the authorised process when entitlement cannot be confirmed.
...
-- Rescore against the corrected framing.
...
-- =========================================================================
--
-- THE Phase 1F defect. scp_item_option_texts.label now carries ONLY the action
-- the participant can choose: no evaluation, no error type, no scoring hint,
-- no indication of which option is preferred.
...
DECLARE _n int;
BEGIN
  -- THE correction: no candidate label may equal its internal rationale.
  ...
  -- Every item classified, every legally flagged item explained.
  ...
  -- sg-b-02 no longer rests on a coercive power.
