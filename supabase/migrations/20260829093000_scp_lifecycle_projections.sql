-- #51 — One lifecycle, three projections.
--
-- The employer Tests pipeline, Medarbetare > Person > Tester & bedömningar, and
-- the participant's own Tester & bedömningar are three views of the same
-- assignment/attempt/report objects. If each derives "what state is this in?"
-- for itself, they drift, and the product tells one person their result is
-- ready while telling another it is still under review.
--
-- So the derivation lives in ONE function and every projection calls it.
--
-- The vocabulary is the product's, not the database's. scp_attempts.status
-- carries in_progress/submitted/scored/released, which are engine words; users
-- see invited / in progress / under review / ready to release / result
-- available. Translating in one place is what keeps the three surfaces honest.
...
-- ═══════════════════════════════════════════════════════════════════════════
-- Projection 1 — the employer's operational pipeline
--
-- Answers, for one organisation: what have we assigned, to whom, why, where is
-- it, and what is the next action. Carries employee_id so a row links straight
-- to that person's profile: the pipeline and the person page are the same
-- objects seen from two directions, not two systems.
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════════
-- Projection 2 — the same rows, one person, one employer
--
-- Rewired onto the shared derivation rather than keeping its own CASE.
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════════
-- Projection 3 — the participant's own history
--
-- Same objects, same derivation, participant audience. Deliberately returns the
-- PARTICIPANT snapshot id and never the employer one: the two audiences are
-- separate reports and mixing them is the failure this model exists to prevent.
-- ═══════════════════════════════════════════════════════════════════════════
