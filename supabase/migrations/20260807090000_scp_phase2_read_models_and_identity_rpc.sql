...
--
-- ADDITIVE ONLY. No content is published, nothing becomes assignable, and no
-- RLS policy on scp_subject_identities, scp_attempts or scp_candidate_responses
-- is added or weakened.
--
-- ── THE ARCHITECTURAL RULE THIS ENCODES ────────────────────────────────
--
-- Read models carry PSEUDONYMOUS subject references and aggregated competence
-- only. Resolving a subject to a person is a separate, explicitly authorised
-- server operation — never a view.
--
-- A security_invoker view joining scp_subject_identities would return nothing
-- (the caller cannot read that table), and a definer view would BE the broadly
-- queryable identity surface we are forbidding. Neither is acceptable, so
-- identity resolution is a single scoped function that requires the caller to
-- name the subject they already legitimately hold a reference to.

-- =========================================================================
-- SECTION 1 — Employer read models, pseudonymous only
-- =========================================================================
--
-- Every one of these is security_invoker, so the caller's own RLS still
-- applies, and none of them touches scp_subject_identities.
...
-- Registered against the published contract. scp_rm_team_competence stays
-- RESERVED: Manager AI is not built, and a contract row must never claim
-- "available" for a view that does not exist.
...
-- =========================================================================
--
-- The ONLY path from a pseudonymous subject to a person, for an employer.
--
-- Deliberately NOT a view and NOT a list. It takes a subject the caller already
-- legitimately holds a reference to and answers one question: may THIS employer
-- see who THIS participant is, right now. Every check must pass; any failure
-- returns zero rows rather than an error, so the function cannot be used as an
-- oracle to probe which subjects exist.
...
DECLARE _n int;
BEGIN
  -- No view anywhere reaches the identity mapping.
  ...
  -- The identity RPC is SECURITY DEFINER and not callable by anon.
  IF NOT EXISTS (SELECT 1 FROM pg_proc
                  WHERE proname = 'scp_resolve_participant_identity' AND prosecdef) THEN
    RAISE EXCEPTION 'SCP_P2_IDENTITY_RPC_NOT_DEFINER';
  END IF;

  -- scp_subject_identities still has no employer-facing policy.
  ...
  -- Every 'available' contract row must have a real view behind it.
  ...
  -- Nothing published, nothing assignable, AI still off.
