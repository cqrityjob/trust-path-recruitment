-- Phase 2c — the test-only published fixture programme.
--
-- ADDITIVE ONLY. is_test_fixture = true on everything published here.
--
-- ── WHAT THIS IS, AND WHAT IT IS NOT ──────────────────────────────────────
--
-- This is NOT the Security Guard programme. That content stays exactly as
-- Phase 1G left it: draft, unreviewed, unpublishable and unassignable.
--
-- This is four deliberately neutral items whose only job is to exercise every
-- supported delivery path end to end -- single-best, best/worst, rated
-- effectiveness and constructed response -- so that the employer and
-- participant journeys can be proven against something real rather than
-- described in a document.
--
-- The scenarios are intentionally generic workplace situations with no legal
-- claim, no regulated terminology and no authority reference, so
-- scp_guard_legal_review_before_publish() does not apply. That is not a way
-- around the gate; it is what makes this fixture safe to publish while the real
-- content waits for the reviewers it genuinely needs.
...
BEGIN
  -- ── Anchors from the existing graph ────────────────────────────────────
  ...
  -- Reuse a real observable behaviour. The fixture proves the PIPELINE; it does
  -- not invent a second taxonomy to prove it against.
  ...
  -- ── The four items, one per supported format ───────────────────────────
  ...
  -- The worst option carries a NAMED professional error, on the same 0-3 scale
  -- as the others. The fixture is held to the same content standard as real
  -- content: a caricature would teach the pipeline nothing.
  INSERT INTO public.scp_item_options
  ...
  -- Item 3 — rated effectiveness (the scale format).
  ...
  -- Item 4 — constructed response. This is the one that must route to a human.
  ...
  -- Publication is a reviewed TRANSITION, never an initial value -- which is
  -- why every row above was inserted as draft and is promoted here.
  ...
  -- ── Report templates, one per audience ─────────────────────────────────
  -- Release refuses to render without these, and a published template must
  -- state its own limitations.
  ...
  -- ── Prove the boundary in the same transaction ─────────────────────────
  ...
  -- Every fixture item is bilingual. Parity is a property of the content, so it
  -- is asserted where the content is written.
  SELECT count(*) INTO _n
    FROM public.scp_form_items fi
