-- The employer and the participant get different reports from the same frozen run.
--
-- ── THE DEFECT ──────────────────────────────────────────────────────────
--
-- scp_release_attempt_report computed the payload ONCE and inserted the same
-- jsonb into both snapshot rows:
--
--   INSERT INTO scp_report_snapshots (... audience, payload ...)
--   VALUES (..., 'participant', COALESCE(_payload,'[]'), ...);
--   INSERT INTO scp_report_snapshots (... audience, payload ...)
--   VALUES (..., 'employer',    COALESCE(_payload,'[]'), ...);
--
-- The two audiences differed only by which limitations template was attached.
-- That is safe exactly as long as the payload contains nothing an employer may
-- see and a participant may not — and it stops being safe the moment the
-- employer report gains reviewer severity, reviewer judgement or interview
-- support, which is what Phase 8 adds.
--
-- The test suite could not catch it either: scp_phase2_journey_test J5.4/J5.5
-- apply the SAME bool_and predicate to both rows, so two byte-identical
-- snapshots pass. Absence was never asserted, only presence of nothing bad.
--
-- ── THE FIX: ALLOWLISTS, NOT FILTERING ─────────────────────────────────
--
-- Each audience payload is BUILT from its own explicit field list. There is no
-- shared object that later has keys removed, and no UI-side hiding: a field the
-- participant may not see is never placed in the participant jsonb at all, so
-- it cannot leak through a template change, a new component or a forgotten
-- filter. Absence is enforced at the point of construction.
--
-- ── EVIDENCE VOCABULARY ────────────────────────────────────────────────
--
-- The Source of Truth requires the words Starkt visat / Visat / Behöver
-- följdfråga / Ännu inte visat / Kritisk följdfråga. The stored maturity scale
-- is a different axis (how strong the EVIDENCE is), and it is deliberately left
-- alone: scp_compute_maturity, scp_maturity_thresholds and every stored value
-- and historical snapshot are untouched. What is added is a versioned
-- projection ON TOP, stamped into the snapshot so an old report can always be
-- re-derived exactly as it was issued.
--
-- Priority, and the reasoning for each step:
--
--   1. critical_follow_up  — ONLY from human-reviewed safety-critical evidence
--      that needs an employer to act: a review still open, or a reviewer who
--      recorded severity high/critical. Never inferred from a low score, because
--      "did poorly" and "did something unsafe" are different claims.
--   2. follow_up           — evidence thin or mixed, or a reviewer who adjusted
--      or overturned the reading and thereby asked for a second look.
--   3. strong_evidence → strongly_shown, consistent_evidence → shown,
--      no_evidence → not_yet_shown.
--   4. limited_evidence / developing_evidence → follow_up. There is no
--      sufficiency rule that makes them "shown": scp_maturity_thresholds puts
--      sufficiency at consistent_evidence (3 observations across 2 contexts),
--      and both of these sit below it.
--
-- CONSEQUENCE, STATED HERE SO IT IS NOT A SURPRISE: consistent_evidence needs
-- min_contexts = 2. A single assessment is one context, so from one run every
-- competency lands on follow_up. That is the threshold model working, not a
-- defect — one 30-minute instrument genuinely is one source. The report says so
-- in its coverage section rather than dressing it up, and a second context
-- (independent scenario, supervisor check, learning evidence) is what makes
-- "shown" reachable.
--
-- Forward-only. Remediation: restore scp_release_attempt_report from
-- 20260813090000, then drop the two new tables and the two new columns. No
-- existing snapshot row is read, rewritten or backfilled by this migration.

-- ═══════════════════════════════════════════════════════════════════════════
-- 1. Curated follow-up prompts
-- ═══════════════════════════════════════════════════════════════════════════
--
-- Deterministic and versioned, never generated. A follow-up question printed in
-- a released report is part of the record, so it has to be reproducible and
-- reviewable like any other content — and it must carry no hint of a scoring
-- key, or it becomes a way to read the answer sheet backwards.
...
-- Prompt text is report content, not test material: it names no option, no key
-- and no rationale. Readable like the rest of the content catalogue; writable
-- only by an authoring principal.
...
-- The employer prompt is an interview/coaching question. The participant one is
-- a reflection prompt in the second person — the same subject, addressed to the
-- person rather than about them.
...
-- ═══════════════════════════════════════════════════════════════════════════
-- 2. Report templates that know what they are describing
-- ═══════════════════════════════════════════════════════════════════════════
--
-- Until now the ONLY templates were 'fixture-employer' and
-- 'fixture-participant', and scp_release_attempt_report picked
-- `ORDER BY version_number DESC LIMIT 1` per audience. So a real released report
-- inherited fixture wording: the live employer report for
-- sg-operational-baseline read "Detta är en testfixtur" about content whose
-- definition has is_test_fixture = false. The report misdescribed itself.
...
-- ═══════════════════════════════════════════════════════════════════════════
-- 3. The versioned evidence-state projection
-- ═══════════════════════════════════════════════════════════════════════════
...
-- The maturity level each state was derived FROM, kept out of both audience
-- payloads.
--
-- Maturity is the internal evidence-strength vocabulary. The product speaks the
-- Source of Truth vocabulary instead, and showing both would ask a reader to
-- reconcile two scales that measure different things. But re-deriving a
-- historical report needs the input, and scp_compute_maturity reads a time
-- window (`max_age_days`), so recomputing it later would not necessarily return
-- what was used on release day.
--
-- So it is frozen here, in a column that getAcademyReport does not select and
-- neither audience receives. Absence from the payload is what makes it absent
-- from the client -- not a decision taken in a component.
ALTER TABLE public.scp_report_snapshots
...
-- ═══════════════════════════════════════════════════════════════════════════
-- 4. Release builds two payloads from two allowlists
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- ── The shared spine ────────────────────────────────────────────────────
  -- Competency, level and how much evidence sits behind it. Identical wording
  -- for both audiences on purpose: a person should read the same sentence about
  -- themselves that their employer reads. What differs is what is ADDED.
  WITH lines AS (
  ...
  -- EMPLOYER allowlist: spine + reviewer-facing follow-up prompt.
  ...
  -- PARTICIPANT allowlist: spine + the FACT that a person read it + a
  -- reflection prompt. No severity, no reviewer rationale, no employer
  -- decision. Those keys are never built into this object, so no template or
  -- component can surface them by accident.
  SELECT jsonb_agg(jsonb_build_object(
           'competency_code',    s.competency_code,
           'competency_name_sv', s.name_sv,
           'competency_name_en', s.name_en,
  ...
  -- The internal derivation record: what each state was computed from.
  SELECT jsonb_agg(jsonb_build_object(
  ...
  -- This column feeds the EMPLOYER snapshot only; see the participant insert.
  ...
  -- Templates: the most specific published one for this governance basis, then
  -- the generic. A closed-test run stops describing itself as a test fixture.
  ...
  VALUES (_attempt_id, _a.subject_id, _a.issuer_organization_id, _pv,
          ...
          -- The participant is told a person reviewed a safety-critical answer
          -- (per competency, in the payload). The reviewer's severity grading is
          -- an internal judgement and is not theirs to receive.
...
-- ═══════════════════════════════════════════════════════════════════════════
-- 5. Progress over time speaks the same vocabulary — and one audience at a time
-- ═══════════════════════════════════════════════════════════════════════════
--
-- Two problems, both surfaced by splitting the payloads:
--
--   * scp_subject_progress read `maturity_level` straight out of the payload.
--     That column has moved to derivation_input, and showing the internal
--     evidence-strength vocabulary next to the product vocabulary would ask a
--     reader to reconcile two scales that measure different things.
--
--   * It selected EVERY snapshot for the subject with no audience filter, so
--     each competency came back twice per release — once from the employer row
--     and once from the participant row. With identical payloads that was
--     invisible; with different ones it is a duplicate row per competency.
--
-- The audience is now chosen the same way the RLS policies choose it: the
-- subject sees their own participant series, an employer member sees the
-- employer series. Nobody sees both.
