-- The recipient payload states each disclosed credential's DEFINITION scope.
--
-- ── THE GAP ────────────────────────────────────────────────────────────
--
-- A shared credential travels to the recipient as a payload built by
-- sp_credential_payload_v2. It carried the claim's jurisdiction, but not
-- whether the DEFINITION is an international professional certification or a
-- national one. A CPP therefore reached the recipient with a null jurisdiction
-- and no way to tell it from a national credential whose country was never
-- stated — so the shared card could not honestly draw the globe, and drew no
-- scope at all (PR #263, "missing data contract").
--
-- ── THE FIX, AND ITS LIMITS ────────────────────────────────────────────
--
-- One key, 'scope_code', read from public.sp_credential_types by the claim's
-- governed code: 'global_professional', 'national_regulated', or NULL when
-- the claim has no governed code. It is the definition's declaration, never
-- derived from a blank field, and NULL stays NULL — an unknown scope is
-- reported as unknown, not guessed.
--
-- EXPAND only: a new key in a JSON payload. Nothing that reads the payload
-- today breaks, and the application treats a missing key exactly as it
-- treated the missing information before (scope not stated). The function's
-- signature, security, grants and every other key are unchanged. Legacy
-- packages built by sp_disclosure_payload_v1 are not changed here.
BEGIN;
