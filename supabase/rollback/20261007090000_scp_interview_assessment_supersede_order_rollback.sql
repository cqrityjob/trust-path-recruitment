...
--
-- Be clear about what that means: it puts back the state in which changing a
-- recorded assessment always fails on
-- scp_interview_assessments_live_idx. Nothing is lost -- assessments already
-- superseded keep their chain, because the chain is data and not behaviour --
-- but the supersede path stops working again.
