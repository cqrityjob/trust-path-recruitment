...
--
-- Reverses 20260914091000_sp_uae_dubai_cadre_catalogue.sql, and ONLY that.
--
...
-- in one of these was not added by 20260914091000 and must survive.
...
BEGIN
  ...
  IF _claims > 0 AND coalesce(_opted_in, '') <> 'yes' THEN
    RAISE EXCEPTION
      ...
      'added by 20260914091000, % of them corrected. This rollback will not '
...
-- ---------------------------------------------------------------------------
-- 2. Exactly the rows 20260914091000 added
-- ---------------------------------------------------------------------------
...
BEGIN
  IF (SELECT count(*) FROM public.sp_credential_types
    ...
    RAISE EXCEPTION
      ...
      'were removed by the rollback of 20260914091000';
  END IF;
