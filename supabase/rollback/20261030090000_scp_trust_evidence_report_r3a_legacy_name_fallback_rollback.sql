-- ROLLBACK for 20261030090000_scp_trust_evidence_report_r3a_legacy_name_fallback.
--
-- Reinstalls the PR-R3A (20261029090000) definition of scp_employer_report_v3
-- verbatim, fallback defect included. Privileges survive CREATE OR REPLACE.
-- Writes no row. Safe to run more than once.
...
BEGIN
  ...
  IF _src NOT LIKE '%INTO _name_sv, _name_en, _cver%' OR _src LIKE '%_fb_sv%' THEN
    RAISE EXCEPTION 'SCP_R3A_FALLBACK_ROLLBACK: the PR-R3A definition was not restored';
  END IF;
  RAISE NOTICE 'ok  PR-R3A fallback hotfix rolled back: the PR-R3A definition is restored';
END $$;
