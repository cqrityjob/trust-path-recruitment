...
-- (verbatim below), no manifest table, no link columns on the snapshots, no
...
-- ── 1. The release function, as 20260830093000 defined it ──────────────────
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
    ...
    RAISE EXCEPTION 'SCP_R1_ROLLBACK: the release function is not the pre-R1 definition';
  END IF;
