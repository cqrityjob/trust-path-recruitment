...
/** The governed locales the notice exists in, exactly as PR 3A publishes them.
 *
 *  This is NOT the application's own language list. `bcp_notice_locales()` is
 *  the authority, the read model carries it verbatim, and a locale outside it
 *  is refused by the database rather than silently answered -- so the UI reads
 *  what the notice actually exists in instead of assuming its own two. */
export type BesktNoticeLocale = string;

/** One locale's governed notice, as `by_locale` carries it.
 *
 *  `noticeContentHash` is bcp_notice_hash(assignment, locale) for THIS locale,
 *  and it is what acknowledgement is bound to: a candidate who read the notice
 *  in Swedish confirms the Swedish bytes, not "the notice". The descriptor
 *  carries the governed values that are data rather than copy -- the retention
 *  class, the lawful basis -- plus the template digest and the key list that
 *  scripts/beskt-notice-copy-digest-check.ts holds the dictionary to. */
export interface BesktNoticeForLocale {
  readonly locale: BesktNoticeLocale;
  readonly noticeContentHash: string;
  readonly sections: readonly string[];
  readonly noticeCopyDigest: string;
  readonly noticeCopyKeys: readonly string[];
  readonly retentionClass: string;
  readonly lawfulBasisReference: string | null;
  readonly jurisdictionReference: string | null;
  readonly recipients: readonly string[];
  readonly decisionMaker: string;
  /** Always false. Read from the descriptor rather than assumed, so that a
...
export interface BesktNotice {
  ...
  readonly locales: readonly BesktNoticeLocale[];
  readonly byLocale: Readonly<Record<BesktNoticeLocale, BesktNoticeForLocale>>;
}
...
export const getMyBesktPreparation = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context, data }): Promise<BesktCandidatePreparation> => {
    ...
    // ── THE NOTICE, PER GOVERNED LOCALE ──────────────────────────────────
    //
    // The read model answers with every locale the notice exists in, each
    // with its OWN content hash and descriptor. Nothing here picks one: the
    // candidate's language choice does that at render time, and the hash that
    // travels back to bcp_acknowledge_notice is the hash of the locale they
    // actually read. Collapsing this to a single notice is what made the
    // earlier "exact notice bytes" claim untrue.
    const locales = ((n.locales as string[] | null) ?? []).slice();
    const rawByLocale = (n.by_locale as Record<string, unknown> | null) ?? {};
    const byLocale: Record<string, BesktNoticeForLocale> = {};
    for (const loc of locales) {
      const entry = rawByLocale[loc] as Record<string, unknown> | undefined;
      if (!entry) continue;
      const desc = entry.descriptor as Record<string, unknown>;
      byLocale[loc] = {
        locale: desc.locale as string,
        noticeContentHash: entry.notice_content_hash as string,
        sections: (desc.sections as string[] | null) ?? [],
        noticeCopyDigest: desc.notice_copy_digest as string,
        noticeCopyKeys: (desc.notice_copy_keys as string[] | null) ?? [],
        retentionClass: desc.retention_class as string,
        lawfulBasisReference: (desc.lawful_basis_reference as string | null) ?? null,
        jurisdictionReference: (desc.jurisdiction_reference as string | null) ?? null,
        recipients: (desc.recipients as string[] | null) ?? [],
