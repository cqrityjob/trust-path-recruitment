...
export function AiDraft({
  ...
  const manifest = z
    .object({
      sources: z.array(z.object({ sourceItemId: z.string(), reviewVersion: z.number() })),
    ...
    <section className={`${panelClass} space-y-5`}>
      ...
      {status.isPending ? (
        ...
        !uncertain && (
          ...
          >
            {l("Skapa AI-utkast från granskat underlag", "Create AI draft from reviewed evidence")}
          </WorkButton>
        )
      )}
      {uncertain && (
              ...
              {output.contradictions.map((c, index) => (
                <article key={index} className="mt-3 space-y-2">
                  ...
                  {c.citations.map((citation, n) => (
                    <blockquote key={n} className="border-l-2 border-border pl-4 text-sm">
                      {citation.quote}
                    </blockquote>
                  ))}
                </article>
                ...
                {output.report.sections.map((section) => (
                  <section key={section.key} className="space-y-3">
                    ...
                    {section.missingInformation && (
                      <p className="text-sm text-muted-foreground">{section.missingInformation}</p>
                    )}
                  </section>
                ))}
              </div>
