...
export function CandidatePreparation({ assignmentId }: { readonly assignmentId: string }) {
  ...
  // ── THE NOTICE THE CANDIDATE IS ACTUALLY READING ─────────────────────
  //
  // The read model carries every governed locale with its own content hash.
  // This resolves the one on screen, and the acknowledgement below sends THAT
  // hash -- so what is recorded is the notice they read, not "the notice".
  // A UI language the notice does not exist in falls back to the first
  // governed locale rather than inventing one, because bcp_acknowledge_notice
  // refuses an ungoverned locale outright.
  const noticeLocale = lang === "sv" ? "sv-SE" : "en-GB";
  const notice = data
    ? (data.notice.byLocale[noticeLocale] ?? data.notice.byLocale[data.notice.locales[0]])
    : undefined;
    ...
    mutationFn: async () => {
      ...
      return acknowledge({
        data: {
          ...
          noticeContentHash: notice.noticeContentHash,
...
export function NoticePanel({
  ...
  /** The governed notice for the locale on screen. Its `sections` and its
   *  governed values come from the descriptor for THAT locale, so the panel
