// Security Career Discovery — the 26-question flow.
//
// ONE implementation, mounted at two paths:
//   /security-career-assessment/session  — canonical
//   /discovery/session                   — temporary alias, redirects here
//
// Path-agnostic: the caller supplies where to send the corrected URL and the
// finished report, so neither mount duplicates the engine.
...
export interface DiscoverySessionProps {
  /** The session id from the route's search params. */
  sessionId: string;
  /** This route's own path, used when replacing a broken URL. */
  sessionPath: string;
  /** Where a finished session sends the candidate. */
  reportPath: string;
}

export function DiscoverySessionView({
  sessionId,
  sessionPath,
  reportPath,
    ...
    loadState({ data: { sessionId: resolved } })
      .then((s) => {
        ...
        if (s.session.status === "completed" && s.snapshotId) {
          navigate({ to: reportPath, params: { snapshotId: s.snapshotId } as never });
          return;
        }
  ...
  }, [sessionId, loadState, startSession, navigate, t, lang, sessionPath, reportPath]);
  ...
  const advance = async () => {
    ...
    if (isLast) {
      ...
      try {
        ...
        navigate({ to: reportPath, params: { snapshotId } as never });
      } catch {
        setStage("questions");
