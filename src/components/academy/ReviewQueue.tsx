...
// Outcome is no longer a type this file needs: deriveOutcome() produces the
// value and completeReview() takes it. Nothing here chooses one.
          ...
          <fieldset className="space-y-3 rounded-[10px] border border-border p-4">
            ...
            {showMethod && (
              <>
                ...
                {/* Emma asked whether AI could do this. The honest answer
                    belongs where somebody asks how the assessment works, not
                    on top of every card -- and it has to be visible somewhere,
                    because a reviewer who assumes they are correcting a
                    machine is reviewing under a false premise. */}
                <p className="text-[12px] leading-relaxed text-muted-foreground">
                  {t(
                    proposal === null
            ...
            >
              ...
              <span className="min-w-0">
                <span className="flex flex-wrap items-baseline gap-x-2">
                  {/* A governed rubric could one day carry a level outside
                      0-4. Falling back to the descriptor is right: it is the
                      authored text, it is already rendered below, and a
                      missing translation key would render as the key. */}
                  <span className="text-[13px] font-semibold text-foreground">
                    {RUBRIC_LEVEL_LABEL_KEY[l.level]
