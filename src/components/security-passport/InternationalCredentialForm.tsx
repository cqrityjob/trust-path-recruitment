    ...
    >
      <summary className="min-h-11 cursor-pointer font-semibold">
        {initial
          ...
          : copy("Lägg till annat yrkesbevis", "Add another professional credential")}
      </summary>
      ...
      <form onSubmit={(e) => void submit(e)} className="grid gap-4 sm:grid-cols-2">
        ...
        {(
          [
            ["title", copy("Ursprungligt namn", "Original credential name")],
            ["issuer", copy("Utfärdare (egen uppgift)", "Issuer (self-reported)")],
            ["identifier", copy("Bevisnummer (valfritt)", "Credential identifier (optional)")],
            ["language", copy("Originalspråk, språkkod", "Original language code")],
          ] as const
        ).map(([key, label]) => (
        ...
        <label className="text-sm">
          {copy("Utfärdarland", "Issuing country")}
          <select
            className={inputClass}
            value={draft.country}
            onChange={(e) =>
          ...
          >
            <option value="">{copy("Inte angivet", "Not stated")}</option>
        ...
        <label className="text-sm">
          {copy("Utfärdande jurisdiktion", "Issuing jurisdiction")}
          <select
            className={inputClass}
        ...
        <label className="text-sm">
          {copy("Giltighetsjurisdiktion (egen uppgift)", "Validity jurisdiction (self-reported)")}
          <select
            className={inputClass}
