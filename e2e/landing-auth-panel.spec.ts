...
test.describe("image 0 · the landing page carries a working login panel", () => {
  ...
  // A signed-in visitor never sees the panel.
  //
  // This asserted the wrong thing first time round: it demanded that a
  // signed-in visitor STAY on /. The homepage has sent authenticated
  // visitors to /my-career since long before this panel existed, and
  // public-homepage.spec.ts pins that ("a signed-in visitor at / is
  // redirected to /my-career, without a loop"). Two suites cannot hold
  // opposite invariants for one route; the older, settled one is right,
  // and this test was the newcomer contradicting it.
  //
  // What belongs here is the half that is actually this panel's business:
  // a signed-in visitor is never shown a sign-in form on the way through.
  test("a signed-in visitor is never shown the panel", async ({ page }) => {
    await page.setViewportSize({ width: 1440, height: 900 });
    await page.goto(`${BASE}/`, { waitUntil: "networkidle" });
