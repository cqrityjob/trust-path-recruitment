...
**Phase 1 of 4. Migration `20261110090000_sp_global_professional_certifications`.**
...
yet; the holder experience is Phase 2's.
...
| The issuer's country | ASIS is incorporated somewhere. A CPP is not a credential *of* that place |
...
way to *say* international and adds no way to *guess* it.
...
`sp_certification_issuers.verification_mode` records what the *issuer*
...
is emphatically *not* `sp_claims.valid_until`, which means one thing across the
...
certify an *organisation's* management system. The Career Center already
...
No UI. No share transport change. No recipient field. No market activated.
Nothing written to hosted Supabase. No Edge Function, no DNS, no Lovable
action, no deployment, no publication. `listGlobalCertificationTypes` exists
and no route or component calls it —
`scripts/passport-global-certification-check.ts` asserts exactly that, so the
day Phase 2 wires it up is a deliberate change rather than a drift.
