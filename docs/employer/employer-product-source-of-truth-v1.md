...
| **MVP** | **Blocked, and honestly blocked.** `selection_support` has no published purpose version, so `scp_employer_assign` refuses recruitment with `SCP_PURPOSE_NOT_AVAILABLE`. `scp_employer_assign` also requires the recipient to already hold an account. |
