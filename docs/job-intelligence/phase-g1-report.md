...
**Status:** Implemented, code-reviewed, locally validated against a real
PostgreSQL 16 instance, and **applied to the Lovable Cloud managed
database per explicit owner approval (Mostafa Alshawi)**. Applied
migration: `supabase/migrations/20260719115332_aa5ec826-c781-4d2d-a03e-f6c744d43272.sql`,
generated by Lovable Cloud's own tooling from the exact, fully-fixed
content of this branch's `20260719100000_employer_memberships.sql` at
commit `0354e07` (confirmed verbatim — see "Cloud application and
post-application verification" below). `src/integrations/supabase/
types.ts` has been regenerated and confirmed to include every new table
and function. Feature flags remain `false`
(`VITE_EMPLOYER_PORTAL_ENABLED`, `VITE_JOBS_ENABLED`,
`VITE_CIG_LIFECYCLE_ENFORCED`). **PR #4 has not been merged; nothing has
been deployed or published.** This revision incorporates three required
corrections from the final Phase G1 security review (verdict: APPROVE
WITH REQUIRED CHANGES) — see "Final active owner protection"
(race-condition fix), "No-op update handling", and "removed_at
semantics" below — plus two further fixes found by local database
execution (RLS recursion, an ambiguous column reference), both of which
are present in the version actually applied to Lovable Cloud.
...
| File | Change |
|---|---|
| `supabase/migrations/20260719100000_employer_memberships.sql` | **New, then updated in place (code-review fix, then local-validation-fix revisions) — kept as one file throughout, since it was never applied under this filename.** Added section 9 (`update_employer_membership`) in the code-review-fix pass, then the `employer_is_active_status` RLS-recursion fix and the `update_employer_membership` alias fix in the local-validation pass. **This exact file's content (as of commit `0354e07`) is the one actually applied to Lovable Cloud — under Lovable's own auto-generated filename, `20260719115332_aa5ec826-....sql` (see "Cloud application" below); this file itself remains unapplied under its own name, which is expected and not a discrepancy.** |
...
## Cloud application and post-application verification

Everything in "Local database validation" above ran against a synthetic
local database, not Lovable Cloud. The migration has since been applied
to the real, connected Lovable Cloud database — this section records
what was independently verifiable from the repository's own commit
history at merge time, folding in Lovable Cloud's own post-application
verification pass rather than duplicating a second, separate report
under the same filename.

**Applied migration:** `supabase/migrations/20260719115332_aa5ec826-c781-4d2d-a03e-f6c744d43272.sql`
(Lovable Cloud's own auto-generated filename). Its header states:
`Approved source: PR #4, branch phase-g1-employer-memberships, commit
0354e07` and `Applied per Owner Approval — Mostafa Alshawi`. **Content
verified, statement by statement, to be byte-identical** to this
branch's `20260719100000_employer_memberships.sql` at commit `0354e07`
— including both fixes found by local validation
(`employer_is_active_status` for the RLS-recursion fix, and the aliased
`em.id` reference inside `update_employer_membership`). No divergent or
re-authored SQL was applied.

**Generated types:** `src/integrations/supabase/types.ts` was
regenerated by Lovable Cloud after the migration applied and confirmed,
by direct diff inspection, to now include: the `employer_memberships`
table (full `Row`/`Insert`/`Update`/`Relationships`, with its FK to
`employers`), `employers.status`, and the `is_platform_admin`,
`has_employer_role`, `employer_is_active_status`, and
`update_employer_membership` function signatures — additive only, no
existing type definition altered. No hand-editing was performed, by
Lovable or by this merge.

**All 8 admin-equivalence policies, verified live against the connected
database** (via a direct `pg_policies` query, per Lovable Cloud's own
report): every one of `employer_admin_meta_admin_all`,
`employers_admin_all`, `job_admin_meta_admin_all`,
`job_audit_events_admin_select`, `job_import_sources admin read`,
`job_import_sources_admin_all`, `jobs_admin_select`, `jobs_admin_write`
now reads `is_platform_admin(auth.uid())` in `qual` (and in
`with_check`, for the `ALL`/write policies) — `has_role(_, 'admin')` no
longer appears in any of them. Matches this report's own migration
design exactly.

**Saved-report smoke test:** a previously saved career report was opened
via `/my-career/reports/<runId>` with the signed-in owner account —
rendered without error, printable layout unaffected, no cross-account
read possible (RLS on `assessment_run_reports` unchanged), no personal
data reproduced in the verification report. Confirms Phase G1 did not
regress the unrelated Phase 2 saved-report surface, consistent with this
report's own "Regression results" above (which established this via
static diff inspection; this is the live confirmation).

**Files actually changed by the Cloud application, per Lovable Cloud's
own commit history:** exactly two — the migration file itself, and
`types.ts` (additive-only). No route, component, `.env`, RLS-unrelated-
to-Phase-G1, CI, or other product-logic file was touched by the
application process itself. (The separate `.env` change visible in this
branch's git history, `abe6259` "Update plan", predates the Phase G1
migration commits and is unrelated to it — confirmed by commit ordering
and an empty diff for `.env` in every Phase-G1-migration-related commit
inspected.)

**Security-linter review:** `supabase--linter` reported 6 findings
against the post-application schema, all reviewed and accepted, not
outstanding risks:
1. `INFO` — RLS enabled, no policy, on `public.audit_logs` — pre-existing
   by design (fail-closed; only `service_role` writes/reads).
2. `WARN` — public/anon can execute a `SECURITY DEFINER` function — the
   pre-existing CIG public-discovery read-helper set, unrelated to Phase
   G1, already documented as an accepted pattern.
3–6. `WARN` — signed-in users can execute a `SECURITY DEFINER` function
   — `has_role`, `is_platform_admin`, `has_employer_role`,
   `employer_is_active_status`. Each is `STABLE SECURITY DEFINER SET
   search_path = public`, takes only typed scalar arguments, returns
   only `boolean`, and is called from RLS policies rather than exposed
   as a general-purpose client RPC for arbitrary use — the same
   "avoids RLS recursion" pattern this report documents throughout.
   `PUBLIC` execute is revoked everywhere except the one function that
   must remain `anon`-callable (`employer_is_active_status`, itself
   returning only a non-sensitive boolean). No new privilege-escalation
   path.

**CI status — stated precisely, not assumed:** Lovable Cloud's own
post-application report records its *local* deterministic checks as
passing (`bunx tsgo --noEmit`, `bun run cie:check`, `bun run kg:check`
all `PASS` in Lovable's environment — consistent with every prior run of
the equivalent commands, `bunx tsc --noEmit`/`cie:check`/`kg:check`, in
this report's own "Tests run and results" and "Local database
validation" sections). **GitHub Actions CI on this exact PR branch has
not been independently confirmed green** — Lovable Cloud's own report
states CI "runs against the PR branch once the auto-regenerated
`types.ts` lands there," i.e. after this merge, and asks the owner to
confirm the green build before merging PR #4. This merge lands that
`types.ts` on the branch; **CI still needs to actually run and be
observed green on the resulting commit** — this is not yet true and is
not asserted as true here.

**Recommendation carried over from Lovable Cloud's own report:**
contingent on (1) `types.ts` present on this branch — true as of this
merge; (2) GitHub Actions green on the resulting commit — **not yet
observed, still open**; (3) feature flags remaining `false` at merge
time — confirmed true. No blocker was found in policy wiring, RLS
behaviour, linter output, or the candidate/saved-report flows.

## Known limitations

- **Types regeneration (resolved)** — `src/integrations/supabase/
  types.ts` has been regenerated by Lovable Cloud and confirmed, by
  direct diff, to include every new table/function. See "Cloud
  application and post-application verification" above. No longer a
  limitation.
- **RLS and concurrency verification (resolved)** — both fully executed:
  first against a real local PostgreSQL 16 instance (see "Local database
  validation" above, including the actual two-process concurrency test,
  which found and led to fixing two real, previously-undetected bugs —
  an RLS mutual-recursion defect between `jobs`/`employers` policies,
  and an ambiguous-column reference inside `update_employer_membership`
  that would have broken every call to it), and the RLS/admin-policy
  portion has since been independently re-verified live against the
  actual applied Lovable Cloud database (see "Cloud application and
  post-application verification" above — all 8 admin-equivalence
  policies confirmed via a direct `pg_policies` query). The concurrency
  test specifically has **not** been repeated against the live Lovable
  Cloud database (only locally) — standard Postgres row-locking
  semantics are not expected to differ on managed Postgres, but this is
  not independently confirmed there.
- **GitHub Actions CI (open, not yet observed on this branch)** — see
  "Cloud application and post-application verification" above. Lovable
  Cloud's local checks passed; the actual GitHub Actions run on the
  merged commit has not yet been confirmed green by any independent
  source in this repository's history at merge time.
