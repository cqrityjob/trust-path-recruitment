...
/* ══════════════════════════════════════════════════════════════════════ */
{
  ...
  for (const lang of ["sv", "en"] as const) {
    ...
    const one = text(
      render(React.createElement(Panels.ReportVersionList, { versions: [v(1, "final")] }), lang),
    );
...
/* ══════════════════════════════════════════════════════════════════════ */
{
  ...
  ok(
    /content_hash_algorithm/.test(insertSlice) && /_hash,\s*'sha256'/.test(insertSlice),
    "8.5 as sha256 on every new finalisation, in the row that is actually stored",
  );

  ok(
    /advertised_role_sv/.test(statements) && /advertised_role_en/.test(statements),
    "8.6 the basis names the ADVERTISED role in both languages",
  );
  ok(
    /'application_id'/.test(statements) && /'job_id'/.test(statements),
  ...
  ok(
    !/job_applications/.test(statements) &&
      !/session_notes/.test(statements) &&
      !/cv_documents/.test(statements) &&
      !/sp_claims/.test(statements),
    "8.12 the builder reads no live application, Passport, CV or note table (ER5.8)",
  );
  ok(
    !/scp_report_snapshots/.test(statements),
    "8.13 and never names the assessment snapshot table (TR12.3)",
  );
  ok(
    !/scp_interview_evidence_proposals/.test(statements),
  ...
  ok(
    !/\b(recommend|ranking|pass_fail|total_score|suitab)/i.test(statements),
  ...
  ok(
    /CREATE OR REPLACE FUNCTION public\.scp_iv_final_report/.test(statements),
  ...
  ok(
    /scp_iv_can_read_case/.test(statements),
