...
# ---------------------------------------------------------------------------
# The jurisdiction-first catalogue rolls back FIRST, newest to oldest:
# Abu Dhabi (20260908096000), the Dubai cadre catalogue (20260908095000), then
# Northern Ireland (20260908094000).
#
...
# ROLLBACK BLOCKED naming the count — which is how this ordering was
# discovered rather than assumed.
# ---------------------------------------------------------------------------
echo "==> Verifying the Abu Dhabi market pack rollback"
set +e
SPAZRB_OUT="$(psql -v ON_ERROR_STOP=1 -q -d "$TEST_DB" \
  -f supabase/rollback/20260908096000_sp_uae_abu_dhabi_market_pack_rollback.sql 2>&1)"
SPAZRB_RC=$?
set -e
...
SPDCRB_OUT="$(psql -v ON_ERROR_STOP=1 -q -d "$TEST_DB" \
  -f supabase/rollback/20260908095000_sp_uae_dubai_cadre_catalogue_rollback.sql 2>&1)"
SPDCRB_RC=$?
set -e
...
SPNIRB_OUT="$(psql -v ON_ERROR_STOP=1 -q -d "$TEST_DB" \
  -f supabase/rollback/20260908094000_sp_uk_vehicle_immobilisation_rollback.sql 2>&1)"
SPNIRB_RC=$?
set -e
