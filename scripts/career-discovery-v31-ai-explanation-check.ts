...
ok(
  fallbackSv.provenance.source === "deterministic_fallback",
  "2.1 source is deterministic_fallback when no aiCall is given",
);
...
ok(
  !("provider" in fallbackSv.provenance) || fallbackSv.provenance.provider === undefined,
  "2.4 no provider/model recorded for a fallback-sourced explanation",
);

const fallbackEn = await generateEnrichedExplanation(buildContext("en"));
ok(
  fallbackEn.text.includes("SOC Analyst"),
  "2.5 English fallback resolves the English title, not a mix of locales",
);
...
ok(
  successResult.provenance.source === "ai",
  "3.2 provenance.source is 'ai' when the call succeeds",
);
ok(
...
ok(
  (await generateEnrichedExplanation(ctxSv, async () => mockAiText)).provenance.generatedAt ===
    null,
...
ok(
  failureResult.provenance.source === "deterministic_fallback",
  "4.1 a throwing aiCall falls back, does not propagate the error",
);
ok(
  failureResult.text.length > 0,
  "4.2 the report still gets valid explanation text after an AI failure",
);

const emptyResult = await generateEnrichedExplanation(ctxSv, async () => "");
ok(
  emptyResult.provenance.source === "deterministic_fallback",
  "4.3 an empty AI response also falls back rather than showing blank text",
);
