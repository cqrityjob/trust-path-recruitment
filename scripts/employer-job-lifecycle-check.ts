...

{
  for (const path of [LIST, HUB]) {
    ...
    const deletable = /const deletable\s*=\s*([^;]+);/.exec(src);
    expect(Boolean(deletable), `C: ${path} has no \`deletable\` rule.`);
    ...
    expect(
      /published_at === null/.test(deletable[1]),
    ...
    // jobs_delete_draft() requires status = 'draft'. A rejected or
    // pending_review advertisement offered a delete is a button that always
    // refuses.
    expect(
      /status === "draft"/.test(deletable[1]),
      `C: ${path} offers delete on something other than a draft, which is the ` +
        `only status jobs_delete_draft() accepts.`,
    );
    // The dead end this pair exists to prevent: exactly one of the two
    // lifecycle endings is offered, and the one that is offered is one the
    // server accepts. Both rules read CLOSEABLE_STATUSES from the module that
    // enforces it, so they cannot drift from it separately.
    const closeable = /const closeable\s*=\s*([^;]+);/.exec(src);
    expect(Boolean(closeable), `C: ${path} has no \`closeable\` rule.`);
    if (!closeable) continue;
    expect(
      /!deletable/.test(closeable[1]),
      `C: ${path} can offer delete and close on the same advertisement. Two ` +
        `buttons for the end of one recruitment is the confusion this work ` +
        `set out to remove.`,
    );
    expect(
      /CLOSEABLE_STATUSES/.test(closeable[1]),
      `C: ${path}'s closeable rule no longer comes from CLOSEABLE_STATUSES, ` +
        `so the interface and closeEmployerJob can disagree about which ` +
        `advertisements can be closed.`,
    );
    // Nothing may be left with neither ending available. The surfaces cannot
    // see assignments or invitations, so the only way they learn a delete is
    // impossible is the refusal itself.
    expect(
      /DELETE_REFUSED_CODES/.test(src),
      `C: ${path} does not react to a refused delete, so a draft the database ` +
        `will not delete has no way to be closed either.`,
    );
  }
}
...
// ---------------------------------------------------------------------------
// G. Every audit action the code writes is one the constraint accepts.
// ---------------------------------------------------------------------------
//
// writeAudit() swallows every failure by design -- the employer's action has
// already succeeded and an audit insert must never undo it -- so a rejected
// audit row is completely silent. That made it possible for
// job_audit_events_action_check to stop accepting 'closed' and
// 'duplicated_from' without anyone noticing, and it would have made the first
// permanent delete in the product the one event with no trace of it.

{
  const fns = read(FUNCTIONS);
  const vocabulary = read("supabase/migrations/20260909094000_job_audit_vocabulary_lifecycle.sql");
  const written = [...new Set([...fns.matchAll(/action: "([a-z_]+)"/g)].map((m) => m[1]))].sort();

  expect(written.includes("deleted"), `G: deleteEmployerJob no longer records an audit action.`);

  for (const action of written) {
    expect(
      vocabulary.includes(`'${action}'`),
      `G: employer-jobs.functions.ts writes action='${action}', which ` +
        `job_audit_events_action_check does not accept. writeAudit() swallows ` +
        `the rejection, so the row simply never appears.`,
    );
  }
}
...
console.log(
  "employer-job-lifecycle:check OK (no window.confirm, one path to archived, " +
    "delete offered only where the database allows it, every refusal has copy " +
    "in sv and en, every audit action is in the vocabulary)",
);
