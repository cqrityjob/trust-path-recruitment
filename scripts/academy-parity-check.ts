...
const blankSv = academy.filter(
  (k) => !String((dictionaries.sv as Record<string, string>)[k] ?? "").trim(),
);
const blankEn = academy.filter(
  (k) => !String((dictionaries.en as Record<string, string>)[k] ?? "").trim(),
);
...
if (missingEn.length)
  problems.push(`${missingEn.length} key(s) missing from en: ${missingEn.slice(0, 8).join(", ")}`);
if (missingSv.length)
  problems.push(`${missingSv.length} key(s) missing from sv: ${missingSv.slice(0, 8).join(", ")}`);
if (blankSv.length)
  problems.push(`${blankSv.length} blank sv value(s): ${blankSv.slice(0, 8).join(", ")}`);
if (blankEn.length)
  problems.push(`${blankEn.length} blank en value(s): ${blankEn.slice(0, 8).join(", ")}`);
